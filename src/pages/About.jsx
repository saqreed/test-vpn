...
export default function About() {
  return (
    <div style={{ overflowX:'hidden' }}>
      <SEO
        title="About CloudVPN — Our Mission, Team & Privacy Values"
        description="Learn about CloudVPN's mission to make military-grade privacy accessible to everyone. Founded in 2018, we protect 10M+ users in 180 countries with a strict zero-log policy."
        path="/about"
        keywords="about CloudVPN, VPN company, VPN privacy mission, zero log VPN company, VPN founded, CloudVPN team, VPN history"
      />
          ...
          <div style={{ display:'flex', alignItems:'center', gap:60, flexWrap:'wrap' }}>
            <FU>
              <div style={{ flex:'1 1 380px' }}>
                <span style={{ fontSize:11, fontWeight:700, letterSpacing:2.5, textTransform:'uppercase', color:'var(--text-2)' }}>Our Mission</span>
                <h2 style={{ fontSize:'clamp(24px,3vw,40px)', fontWeight:900, color:'var(--text)', letterSpacing:'-0.6px', margin:'12px 0 16px' }}>
                  Privacy for everyone, not just the privileged
                </h2>
                <p style={{ fontSize:16, color:'var(--text-3)', lineHeight:1.75, marginBottom:14 }}>
                ...
                <p style={{ fontSize:16, color:'var(--text-3)', lineHeight:1.75, marginBottom:28 }}>
              ...
              <div style={{ flex:'1 1 300px' }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                  {[['10M+','Users Protected'],['180+','Countries Served'],['100+','Server Locations'],['0','Data Breaches']].map(([v,l]) => (
                    <div key={l} style={{ background:'var(--bg-stat)', borderRadius:16, padding:'24px 16px', textAlign:'center' }}>
                      <div style={{ fontSize:28, fontWeight:900, color:'var(--text-2)', letterSpacing:'-0.8px' }}>{v}</div>
                      <div style={{ fontSize:12, color:'var(--text-3)', fontWeight:500, marginTop:4 }}>{l}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* values */}
      <section style={{ background:'var(--bg-sect)', padding:'90px 0' }}>
        <div style={{ ...W }}>
          <FU>
            <div style={{ textAlign:'center', marginBottom:50 }}>
              <span style={{ fontSize:11, fontWeight:700, letterSpacing:2.5, textTransform:'uppercase', color:'var(--text-2)' }}>Our Values</span>
              <h2 style={{ fontSize:'clamp(24px,3vw,40px)', fontWeight:900, color:'var(--text)', letterSpacing:'-0.6px', marginTop:10 }}>What we stand for</h2>
            </div>
          </FU>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))', gap:16 }}>
            {VALUES.map(({ Icon, title, desc }, i) => (
              <FU key={title} delay={i*0.07}>
                <div style={{ background:'var(--bg-card)', borderRadius:18, padding:'30px 24px', border:'1px solid var(--border)', transition:'box-shadow 0.22s,transform 0.22s', cursor:'default' }}
      ...
      <section style={{ background:'var(--bg)', padding:'90px 0' }}>
        <div style={{ maxWidth:720, margin:'0 auto', padding:'0 28px' }}>
          <FU>
            <div style={{ textAlign:'center', marginBottom:50 }}>
              <span style={{ fontSize:11, fontWeight:700, letterSpacing:2.5, textTransform:'uppercase', color:'var(--text-2)' }}>Our Story</span>
              <h2 style={{ fontSize:'clamp(24px,3vw,40px)', fontWeight:900, color:'var(--text)', letterSpacing:'-0.6px', marginTop:10 }}>The CloudVPN journey</h2>
            </div>
          </FU>
          <div style={{ position:'relative' }}>
            <div style={{ position:'absolute', left:20, top:0, bottom:0, width:2, background:'linear-gradient(to bottom,#3b82f6,#bfdbfe)' }} />
              ...
              <FU key={year} delay={i*0.06}>
                <div style={{ display:'flex', gap:28, marginBottom:32, paddingLeft:8 }}>
                  ...
                  <div style={{ paddingTop:6 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:'var(--text-2)', marginBottom:4 }}>{year}</div>
                    <p style={{ fontSize:14, color:'var(--text-3)', lineHeight:1.65 }}>{event}</p>
                  </div>
                </div>
              </FU>
            ))}
      ...
      <section style={{ background:'var(--bg-sect)', padding:'90px 0' }}>
        <div style={{ ...W }}>
          <FU>
            <div style={{ textAlign:'center', marginBottom:50 }}>
              <span style={{ fontSize:11, fontWeight:700, letterSpacing:2.5, textTransform:'uppercase', color:'var(--text-2)' }}>The Team</span>
              <h2 style={{ fontSize:'clamp(24px,3vw,40px)', fontWeight:900, color:'var(--text)', letterSpacing:'-0.6px', marginTop:10 }}>The people behind CloudVPN</h2>
            </div>
          </FU>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))', gap:16 }}>
            {TEAM.map(({ name, role, initials, desc }, i) => (
              <FU key={name} delay={i*0.06}>
                <div style={{ background:'var(--bg-card)', borderRadius:18, padding:'28px 22px', border:'1px solid var(--border)', transition:'box-shadow 0.22s,transform 0.22s', cursor:'default' }}
                  onMouseEnter={e=>{ e.currentTarget.style.boxShadow='0 10px 28px rgba(59,130,246,0.12)'; e.currentTarget.style.transform='translateY(-3px)' }}
                  onMouseLeave={e=>{ e.currentTarget.style.boxShadow='none'; e.currentTarget.style.transform='translateY(0)' }}>
                  ...
                  <div style={{ fontSize:13, color:'var(--text-2)', fontWeight:500, margin:'4px 0 10px' }}>{role}</div>
